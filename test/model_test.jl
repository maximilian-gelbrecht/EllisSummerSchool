...
@testset "TwoLayerLorenz96" begin
    ...
    @test sol[end] != sol[1]         # something happended 
    @test all(isfinite.(sol[end]))   # no blow up for default config 
    @test (sum(abs.(sol[end])) .> 1) # no fixed point for the default config 
end
